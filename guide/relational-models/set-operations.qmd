...
## Cartesian Product $\times$ in Relational Algebra
...
The following are valid examples of a cartesian project written in unicode:
...
# Define the second dataframe, courses2, with two rows
data = {
    'CID': [6, 7],
    'Course': ['CMSC460', 'CMSC455'],
...
and *Courses( CID,Course,Term,Enrl)*:
...
:::
::: {.guide-block-right}
*Students* $\times$ *Courses* returns:

```{python}
#| echo: false

cartesian_product = pd.merge(students, courses, how='cross')
show_df( cartesian_product )
```
:::

### Cartesian Project - × - Example 2
...
:::
::: {.guide-block-right}
*Courses* $\times$ *Students* returns:

```{python}
#| echo: false

cartesian_product = pd.merge(courses,students, how='cross')
show_df( cartesian_product )
```
:::
