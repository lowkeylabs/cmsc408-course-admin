...
spreadsheet_key = "13-susrPV2Rxk4wvX44jUiwzwS5BNMLvncmlIOFosMUk"
canvas_course_id = 85746
template_qmd_file = "../modules/_lect-template.qmd"
debug_level = "INFO"
...
def reformat_date(date_str):
    """ takes a date string line 1/9 and returns with leading 0 like 01/09.
...
def add_lecture_file_names( lecture_df ):
    ...
    lecture_df["basename"] = "lect" + lecture_df["Lecture ID"].apply( lambda x:f"{int(x):02d}")
...
def sync_files_and_lectures_and_canvas():
    ...
    lecture_files = [filename for filename in os.listdir(".") if os.path.isfile(filename) and re.match("lect\d\d", filename) and os.path.splitext(filename)[1]==".qmd" ]
